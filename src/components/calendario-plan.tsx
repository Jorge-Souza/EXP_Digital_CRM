...
function PostCardContent({ post }: { post: Post }) {
  ...
  return (
    <>
      <p className="text-[9px] font-black uppercase tracking-wide leading-none" style={{ color: tc.text }}>
        {TYPE_LABELS[post.tipo] ?? post.tipo}
      </p>
      <p className="text-[10px] leading-snug mt-0.5 line-clamp-3" style={{ color: tc.text }}>
...
function DraggablePost({
  ...
  const inner = onPostClick ? (
    ...
    <div className="rounded p-1" style={{ background: tc.bg }}>
      <PostCardContent post={post} />
    </div>
  )
...
function DroppableDay({
  ...
  const { setNodeRef, isOver } = useDroppable({ id: dateKey, disabled: !droppable })

  return (
    <div
      ref={setNodeRef}
      className={`group border-r border-t border-gray-200 flex flex-col transition-all ${
        isOver && droppable
          ? "ring-2 ring-inset ring-primary"
          : ""
      }`}
      style={{ background: isOver && droppable ? "#f0f9ff" : cellBg }}
    >
      {children}
    </div>
  )
...
export function CalendarioPlan({
  ...
  async function handleDragEnd(event: DragEndEvent) {
    setActivePost(null)
    if (!onMovePost || !event.over) return
    const postId = event.active.id as string
    const newDate = event.over.id as string
    const post = posts.find((p) => p.id === postId)
    if (!post || post.data_publicacao === newDate) return
    await onMovePost(postId, newDate)
  }

  const isDragEnabled = !!onMovePost
  ...
  return (
    ...
    >
      <div className="overflow-hidden rounded-b-xl">
        ...
        <div className="grid grid-cols-7" style={{ background: "#1f2937" }}>
          {DIAS_SEMANA.map((d, i) => (
            <div
              key={d}
              className={`py-2.5 text-center text-xs font-bold text-white uppercase tracking-widest ${i < 6 ? "border-r border-gray-600" : ""}`}
            >
              {d}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7 border-l border-b border-gray-200">
          {celulas.map((dia, idx) => {
            ...
            const headerBg = evento ? evento.palette.bg : feriado ? "#bae6fd" : isWeekend ? "#f1f5f9" : "#ffffff"
            const headerTextColor = evento ? evento.palette.text : feriado ? "#075985" : "#6b7280"
            ...
            return (
              <DroppableDay
                key={key}
                dateKey={key}
                droppable={isDragEnabled}
                cellBg={cellBg}
              >
                {/* Faixa de cabeçalho do dia */}
                <div className="flex items-center gap-1 px-1.5 py-1" style={{ background: headerBg }}>
                  <span
                    className={`text-[11px] font-black w-5 h-5 flex items-center justify-center rounded-full shrink-0 ${isToday ? "bg-primary text-primary-foreground" : ""}`}
                  ...
                  {(evento || feriado) && (
                    <span className="text-[9px] font-bold truncate flex-1 leading-tight" style={{ color: headerTextColor }}>
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-2 px-5 py-3 border-t border-gray-200 bg-gray-50">
            {Object.entries(TYPE_LABELS).map(([tipo, label]) => (
              <div key={tipo} className="flex items-center gap-1.5">
                <span className="h-2.5 w-2.5 rounded-sm shrink-0" style={{ background: TYPE_COLORS[tipo]?.bg ?? "#f3f4f6" }} />
                <span className="text-xs text-muted-foreground">{label}</span>
              </div>
            ))}
            {datasAtivas.map((d, idx) => (
              <div key={d.data} className="flex items-center gap-1.5">
                <span className="h-2.5 w-2.5 rounded-sm shrink-0" style={{ background: DATA_PALETTE[idx % DATA_PALETTE.length].bg }} />
      ...
      <DragOverlay dropAnimation={null}>
        {activePost ? (() => {
          const tc = TYPE_COLORS[activePost.tipo] ?? { bg: "#f3f4f6", text: "#374151" }
          return (
            <div
              className="rounded p-1.5 shadow-xl rotate-2 w-32 opacity-95"
              style={{ background: tc.bg }}
            >
              <p className="text-[9px] font-black uppercase tracking-wide leading-none" style={{ color: tc.text }}>
                {TYPE_LABELS[activePost.tipo] ?? activePost.tipo}
              </p>
              <p className="text-[10px] leading-snug mt-0.5 line-clamp-2" style={{ color: tc.text }}>
                {activePost.tema || activePost.titulo}
              </p>
            </div>
          )
        })() : null}
